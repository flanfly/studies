...
Xingyi Li a, b, Yujun Liu <sup>a</sup> , Zhuang Liu <sup>a</sup> , Shushang Zhu a,\*

<sup>a</sup> *School of Business, Sun Yat-sen University, 135 Xingang Road West, Guangzhou, 510275, China*

<sup>b</sup> *Center for Financial Engineering and Risk Management, Sun Yat-sen University, 135 Xingang Road West, Guangzhou, 510275, China*

### **Abstract**

We focus on predicting cryptocurrency returns through an analysis employing machine learning techniques. A comprehensive set of factors is constructed, and a battery of 12 machine learning models is used to forecast cryptocurrency returns. We assess the importance of these factors and their influence on model outputs using the advanced SHAP method. Additionally, long-short and long-only cryptocurrency portfolios are constructed to further explore the practical implications of the predictive models. The results highlight the superior performance of tree models, particularly the random forest algorithm, in comparison to neural networks. Compared to the stock market, tree models demonstrate enhanced predictabilities for cryptocurrency returns. Both tree and neural network models identify the market value to realized value ratio, new addresses, and active addresses as the most important factors. The larger these factors, the greater the predicted returns. Notably, portfolios constructed based on neural networks achieve the highest cumulative returns. These insights provide valuable guidance for investors, traders, and financial analysts, informing their decision-making processes within the dynamic and complex landscape of the cryptocurrency market.

**Keywords:** Cryptocurrency; Return prediction; Machine learning; Factor investing; Portfolios
...
Our cryptocurrency factors are composed of macroeconomic and characteristic factors. Based on the fundamental data, we build 17 cryptocurrency characteristic factors spanning 8 dimensions, including network, financial, ownership, exchange, and social. The details of these predictors are provided in the Appendix. We also construct 8 macroeconomic factors across uncertainty indices, market indices, commodity prices, and cryptocurrency index. The construction of the cryptocurrency index is provided in the Appendix. Table 1 summarizes the macroeconomic and characteristic factors.

<sup>1</sup> We predict cryptocurrencies' weekly returns since the fundamental data available is only weekly.

**Table 1** Cryptocurrency factors.

|                     | Characteristic factors            |                                                       |                                                                  |
|---------------------|-----------------------------------|-------------------------------------------------------|------------------------------------------------------------------|
| Dimension           | 1: Network                        | Change rates of volume (TV)                           | new addresses (NA), active addresses                             |
| Dimension           | 2: Finance                        | Change rates of                                       | market value to realized value ratio (MRR)                       |
|                     |                                   | and network value to transaction ratio (NTR)          |                                                                  |
| Dimension           | 3: Ownership                      | Change rates of                                       | large holders netflow (LHN), large holders                       |
|                     |                                   | inflow (LHI), and                                     | large holders outflow (LHO)                                      |
...
| Commodity prices    |                                   | Returns of                                            | the gold price (GLD) from COMEX and the                          |
|                     |                                   | crude oil                                             | price (WTI) from NYMEX                                           |
| Cryptocurrency      | index                             | Return of                                             | Google Trends cryptocurrency index (GTCX)                        |

Table 2 presents the descriptive statistics for cryptocurrency factors. Large holders inflow (LHI) and large holders outflow (LHO) are notable for their large means, with only net flows (NF) exhibiting a negative mean. Most characteristic factors demonstrate considerable standard deviations, especially for large holders netflow (LHN) and NF. The number of transactions (NT), active addresses (AA), and active addresses ratio (AAR) stand out for their markedly high kurtosis. Notably, all factors show significant ADF values at the 1% level, indicating that they are stationary time series. Moreover, the correlations among cryptocurrency factors are provided in Figure 1, revealing significantly low correlation coefficients. Notably, the correlation between AA and AAR is exceptionally high at 0.91. AAR and NT have a moderate correlation of 0.74, while SPX and VIX exhibit a negative correlation of −0.39.

**Table 2**

The descriptive statistics for cryptocurrency factors.

|      | Mean   | Std    | Min    | Max     | Skewness | Kurtosis | ADF     |
|------|--------|--------|--------|---------|----------|----------|---------|
...
| RMRF | 0.029  | 0.955  | −10.61 | 55.838  | −6.065   | 541.834  | −17.851 |
| SMB  | 0.048  | 0.949  | −9.308 | 60.047  | −7.450   | 363.252  | −20.835 |
| UMD  | 0.110  | 0.667  | −0.969 | 67.000  | 5.972    | 382.549  | −14.260 |
...
Furthermore, we assume the conditional expectation of cryptocurrency  $i$ 's return,  $r_{i,t+1}$ , given the information available at period  $t$ , is a constant function of a specified set of predictors:

$$\mathbb{E}_t[r_{i,t+1}] = g(z_{i,t}), \quad (2)$$

where  $z_{i,t}$  is a  $P$ -dimensional vector of predictors, cryptocurrencies are indexed by  $i = 1, 2, \dots, N$ , weeks are indexed by  $t = 1, \dots, T$ . The functional form of  $g(\cdot)$  is left unspecified. We aim to search for the prediction model that gives the best prediction performance from a set of candidates.
...
We follow the standard method in the literature for hyperparameter selection, model estimation, and performance evaluation. Specifically, we divide our data into three disjoint periods while maintaining chronological order: the training sample (2014-2017), the validation sample (2018-2020), and the testing sample (2021-2023). In the training sample, we employ prespecified hyperparameters for a given machine learning model to estimate the model parameters. The validation sample is then utilized to fine-tune the hyperparameters of the machine learning model. We select hyperparameters that minimize the objective loss function based on observations in the validation sample. The testing sample serves the purpose of evaluating the prediction performance of the machine learning model. Notably, these data have not been used in either the model estimation or tuning phases, ensuring an unbiased assessment of the models' predictive capabilities. We standardize the inputs across the training, validation, and testing samples to facilitate comparability and robustness using the  $Z$ -score.
...
where  $\mathcal{T}_3$  indicates that fits are only assessed on the testing sample, whose data never enter model estimation or tuning;  $\hat{r}_{i,t}$  are predicted weekly returns. The results for different models and subsamples are summarized in Table 3.

#### 3.1.1 Full sample analysis

predictive capabilities. Notably, tree models, particularly the RF model, outperform other machine learning models. The OLS model achieves a �������� <sup>2</sup> of 3.901%, suggesting that even a simple linear model retains certain predictability. It is noteworthy that the �������� 2 of the OLS-3 model is significantly lower than that of the OLS model (0.044% vs. 3.901%), indicating that relying only on market, size, and momentum factors is insufficient to account for the entire predictive power in the linear model.

The regularized models do not demonstrate an improvement in their �������� 2 compared to the linear models, suggesting that the factors used are all important and do not need dimension reduction. In the regularized models, both the PLS and Ridge models share an identical �������� 2 (3.901% ), mirroring that of the OLS model. However, the LASSO and ENet models show no predictive power, recording a negative �������� <sup>2</sup> of −0.000%.

The tree models, RF and GBRT, exhibit formidable predictive capabilities that elevate the �������� 2 to surpass 20%. Notably, RF slightly outperforms GBRT, boasting a higher �������� 2 (25.265 v.s. 24.911). Such improvement demonstrates the superiority of machine learning in capturing intricate interactions among predictors in the cryptocurrency market. This advantage aligns with similar findings for stock markets, as highlighted in both Gu et al. (2020) and Leippold et al. (2022).

Although the neural network models also raise their �������� 2 to exceed 15%, they are inferior to the tree models. Among the five neural network models, NN4 exhibits the highest �������� 2 at 18.704%, suggesting that the model with four hidden layers has the best predictive performance. Note that when increasing hidden layers in neural networks, the �������� <sup>2</sup> will also increase, but this improvement is limited to the first four hidden layers. The �������� 2 (18.478) of NN5 is slightly lower than that of NN4. Furthermore, the predictive capabilities of neural network models exhibit notable stability, with their �������� 2 consistently ranging from 15% to 19%. The KNN model performs less effectively than tree and neural network models but exceeds the regularized and linear models. The �������� <sup>2</sup> of the KNN model is 15.831%, slightly lower than NN1 (15.837%).

In addition to the out-of-sample �� 2 , we also employ the Diebold and Mariano (1995) test to conduct comparisons of methods, evaluating the differences in out-of-sample predictive accuracy between two models. The results of the Diebold-Mariano test are presented in Table 4. Our findings are consistent with those derived from the out-of-sample �� 2 , indicating that utilizing the out-ofsample �� 2 for model selection is robust in the cryptocurrency market.

It is noteworthy that the cryptocurrency market is driven by certain popular cryptocurrencies, typically categorized as mega-cap cryptocurrencies. The model predictability for mega-cap cryptocurrencies may differ from that for the full sample, as well as from large-cap and mid-cap cryptocurrencies. This motivates us to consider subgroup analysis.

**Table 3** Weekly out-of-sample predictive �� 2 in percentage.

|       | All    | Mega-cap | Large-cap | Mid-cap |
|-------|--------|----------|-----------|---------|
...
The third column of Table 3 summarizes the �������� 2 for mega-cap cryptocurrencies. The results suggest that all models have a much better predictive performance compared to the full sample.

Linear models nearly double their performance, with OLS achieving a larger  $R_{oos}^2$  than OLS-3 (17.884% vs. 14.035%). This means that the inclusion of only three factors (market, size, and momentum) remains insufficient to explain all predictive power. Regularized models, including PLS and Ridge, experience an increase in  $R_{oos}^2$  (17.884% vs. 17.887%) to surpass 15%. It is worth noting that the  $R_{oos}^2$  of Ridge is slightly larger than that of OLS, indicating redundancy in certain factors for predicting weekly returns of mega-cap cryptocurrencies.
...
Comparison of weekly out-of-sample prediction using Diebold-Mariano test.

| OLS-3 PLS OLS −0.677 0.000 OLS-3 0.677 PLS | LASSO Ridge ENet −0.684 0.000 −0.684 −0.008 0.677 −0.008 −0.684 0.000 −0.684 | RF 3.748 * 4.425 * 3.748 * | GBRT 3.686 * 4.363 * 3.686 * | KNN 2.093 * 2.770 * 2.093 * | NN1 2.094 * 2.771 * 2.094 * | NN2 2.374 * 3.051 * 2.374 * | NN3 2.101 * 2.778 * 2.101 * | NN4 2.597 * 3.274 * 2.597 * | NN5 2.557 * 3.234 * 2.557 * |
|--------------------------------------------|------------------------------------------------------------------------------|----------------------------|------------------------------|-----------------------------|-----------------------------|-----------------------------|-----------------------------|-----------------------------|-----------------------------|
| LASSO                                      | 0.684 0.000                                                                  | 4.432 *                    | 4.370 *                      | 2.777 *                     | 2.778 *                     | 3.059 *                     | 2.785 *                     | 3.281 *                     | 3.242 *                     |
| Ridge                                      | −0.684                                                                       | 3.748 *                    | 3.686 *                      | 2.093 *                     | 2.094 *                     | 2.374 *                     | 2.101 *                     | 2.597 *                     | 3.234 *                     |
| ENet                                       |                                                                              | 4.432 *                    | 4.370 *                      | 2.777 *                     | 2.778 *                     | 3.059 *                     | 2.778 *                     | 3.281 *                     | 3.242 *                     |
| RF                                         |                                                                              |                            | −0.062                       | −1.655                      | −1.654                      | −1.374                      | −1.647                      | −1.151                      | −1.191                      |
| GBRT                                       |                                                                              |                            |                              | −1.593                      | −1.592                      | −1.312                      | −1.585                      | −1.089                      | −1.129                      |
| KNN                                        |                                                                              |                            |                              |                             | 0.001                       | 0.281                       | 0.008                       | 0.504                       | 0.464                       |
| NN1                                        |                                                                              |                            |                              |                             |                             | 0.280                       | 0.007                       | 0.503                       | 0.463                       |
| NN2                                        |                                                                              |                            |                              |                             |                             |                             | −0.273                      | 0.223                       | 0.183                       |
| NN3                                        |                                                                              |                            |                              |                             |                             |                             |                             | 0.496                       | 0.456                       |
| NN4                                        |                                                                              |                            |                              |                             |                             |                             |                             |                             | −0.040                      |

Notes: This table reports pairwise Diebold-Mariano test statistics comparing the out-of-sample prediction performance based on the full sample among all models. Positive (negative) numbers indicate the column (row) model outperforms the row (column) model. Asterisks indicate the difference is significant at 5% level.
...
### 3.1.4 Mid-cap cryptocurrencies

The last column of Table 3 shows the  $R_{oos}^2$  for mid-cap cryptocurrencies. The predictive performance of all models continues to decline. The  $R_{oos}^2$  of OLS (3.977%) is marginally larger than that of the full sample but only half of large-cap cryptocurrencies and one-fifth of mega-cap cryptocurrencies. Notably, the  $R_{oos}^2$  (-0.145%) of OLS-3 turns to negative for the first time, indicating that OLS using only three factors has no predictive power for mid-cap cryptocurrencies. In regularized models, PLS and Ridge maintain the same  $R_{oos}^2$  (3.977%), slightly larger than that of the full sample. The  $R_{oos}^2$  of LASSO and ENet (-0.017%) is slightly lower than that of the full sample.

Surprisingly, the predictive performance of tree and neural network models demonstrates a significant drop, even lower than that of the full sample. The  $R_{oos}^2$  of RF (18.277%) continues to exceed that of GBRT (13.624%), with a smaller gap between them. Among neural network models, NN2 has the largest  $R_{oos}^2$  (15.326%), while NN4 has the smallest (12.053%). This implies that increasing hidden layers in neural networks does not enhance the  $R_{oos}^2$  of the model. Compared to mega-cap and large-cap cryptocurrencies, the difference in the predictabilities of neural network models becomes smaller, with  $R_{oos}^2$  ranging from 12% to 16%.

Achieving  $R_{oos}^2$  of 13.726%, the predictive performance of KNN outperforms GBRT, NN1, NN3, and NN4. The predictability of KNN experiences a notable decrease compared to mega-cap and large-cap cryptocurrencies. Notably, the  $R_{oos}^2$  of KNN is smaller than that of the full sample. The Diebold-Mariano tests for mid-cap cryptocurrencies are summarized in the Appendix. The findings are consistent with those obtained from the out-of-sample  $R^2$ .
...
We employ the SHAP method to investigate the importance of cryptocurrency factors in the full sample. 2 Given the superior predictive performance of tree and neural network models, our emphasis is directed toward these particular model types. Furthermore, we conduct an analysis of the resemblances and variations in the factor importance across distinct tree or neural network models. The results of factor importance for mega-cap, large-cap, and mid-cap cryptocurrencies are provided in the Appendix. Figure 22 shows the importance of factors in tree and neural network models. Overall, the change rates of market value to realized value ratio (MRR), new addresses (NA), and active addresses (AA) are the three most influential factors for predicting weekly returns in the cryptocurrency market.
...
SHAP is a game theoretic approach to explain the output of any machine learning model. The details of the SHAP method are provided in the Appendix.

![](_page_14_Figure_0.jpeg)

**Figure 2.** The importance of cryptocurrency factors in tree and neural network models.

Notes: The horizontal axis denotes the average absolute value of the SHAP values, serving as a metric for factor importance. The larger the value, the more important the factor. The vertical axis represents the factors' names, arranged in descending order based on their importance levels.

## 3.4 Factor effects on model output

We also apply the SHAP method to understand the decision-making processes of tree and neural network models in the full sample. This methodology enables the examination of the impact of cryptocurrency factors on model output, as illustrated in Figure 3. The corresponding results for mega-cap, large-cap, and mid-cap cryptocurrencies are provided in the Appendix.
...
Figure 4 visualizes the contribution of cryptocurrency factors to model prediction, utilizing RF, GBRT, and NN4 as representative examples. These instances offer valuable insights into how each factor drives the forecast toward its final value.

![](_page_16_Figure_0.jpeg)

**Figure 3.** The impact of cryptocurrency factors on tree and neural network models.
...
**Figure 4.** Examples of cryptocurrency factors' contribution to machine learning models.
...
So far, our evaluation of predictive performance has been grounded in statistical comparisons, utilizing the out-of-sample predictive �� 2 and Diebold-Mariano test. We next analyze whether this predictability can be implemented in cryptocurrency portfolios by investigating the out-of-sample performance of value-weighted portfolios and the evolution of their cumulative returns over time. We also study equal-weighted portfolios, the results of which are provided in the Appendix.

We consider two types of cryptocurrency portfolios. The first type is long-short portfolios, which are constructed by following the schemes of Gu et al. (2020) and Leippold et al. (2022). Specifically, we employ each method at the end of each week to predict the out-of-sample returns of cryptocurrencies for the next week. The cryptocurrencies are then divided into deciles based on the predicted returns, and both value-weighted and equal-weighted portfolios are constructed. <sup>3</sup> Thus, a zero-net-investment portfolio is built by buying the cryptocurrencies with the highest expected returns (decile 10) and selling those with the lowest (decile 1). It is noteworthy that implementing long-short portfolios is difficult in the current cryptocurrency market due to the absence of a shortselling mechanism, although they are a useful tool for assessing the portfolio-level performance of machine learning methods. Hence, we also consider long-only portfolios, which only buy the cryptocurrencies in the top decile.

Table 5 reports the out-of-sample performance for value-weighted long-short and long-only

<sup>3</sup> We construct value-weighted portfolios by weighting the market capitalizations of the cryptocurrencies.

portfolios, suggesting that machine learning portfolios dominate linear ones. Overall, machine learning techniques, especially PLS and NNs, are beneficial for portfolio-level predictions. In longshort portfolios, linear portfolios exhibit inferior performance compared to their machine-learning counterparts. The OLS-3 portfolio stands out with the smallest mean and the largest maximum drawdown, indicating that OLS utilizing only three factors is unfavorable for predicting portfolio returns. The NN5-based portfolio records the largest mean, followed by the NN4-based portfolio. Thus, the neural network portfolio is the most proficient when evaluating investment returns. Notably, the Sharpe ratios of neural network portfolios are not the highest, whereas the PLS-based portfolio surprisingly achieves the largest Sharpe ratio and the smallest standard deviation. Furthermore, the NN1-based portfolio exhibits the smallest maximum drawdown, trailed by the RFbased portfolio.
...
The dynamic evolution of cumulative returns for both long-short and long-only value-weighted portfolios is shown in Figure 5. Note that we select NN5-based and NN4-based portfolios as representatives for neural network portfolios in long-short and long-only categories since they are optimal in their respective neural network portfolio types. The dynamics of cumulative returns for long-short and long-only portfolios based on NN1-NN5 will be discussed later.

The cumulative return performance shows that long-short portfolios can be divided into 4 tiers. The first tier contains portfolios based on NN5, GBRT, and RF, demonstrating significantly higher cumulative returns than other portfolios. Within this tier, the NN5-based portfolio achieves the largest cumulative return, followed by the GBRT-based portfolio. The second tier comprises portfolios based on KNN and Ridge, with the former's cumulative return notably exceeding the latter's. Notably, there is a substantial gap between the first and second tiers. Portfolios based on PLS, OLS, LASSO, and ENet are composed of the third tier, with a certain gap between the second and third tiers. Among these portfolios, the PLS-based portfolio attainsthe largest cumulative return, trailed by the OLS-based portfolio. The cumulative returns of portfolios based on LASSO and ENet are nearly the same. Finally, the OLS-3 portfolio forms the fourth tier and exhibits the smallest cumulative return. The gap between the last two tiers has increased markedly since the third quarter of 2021.
...
**Table 5** The out-of-sample performance for value-weighted portfolios on the full sample.

| Long-short    | Mean   | Std    | Sharpe  | Skewness | Kurtosis | MaxDD   |
|---------------|--------|--------|---------|----------|----------|---------|
| OLS           | 13.836 | 19.887 | 241.008 | 0.508    | −0.575   | 100.096 |
| OLS-3         | 7.450  | 21.113 | 122.235 | −0.196   | 2.573    | 187.746 |
| PLS           | 15.654 | 18.955 | 286.080 | 0.430    | −0.572   | 98.702  |
| LASSO         | 14.635 | 23.185 | 218.668 | 4.156    | 25.555   | 99.640  |
| ENet          | 14.587 | 23.131 | 218.464 | 4.177    | 25.812   | 99.829  |
| Ridge         | 22.031 | 28.757 | 265.388 | 1.454    | 2.880    | 99.331  |
| RF            | 32.858 | 43.726 | 260.310 | 1.739    | 3.432    | 98.209  |
| GBRT          | 34.379 | 45.605 | 261.138 | 1.726    | 3.272    | 98.284  |
| KNN           | 25.005 | 32.756 | 264.435 | 1.659    | 3.809    | 99.290  |
| NN1           | 31.991 | 43.669 | 253.772 | 1.823    | 3.629    | 97.533  |
| NN2           | 31.779 | 45.990 | 239.369 | 2.643    | 10.115   | 99.280  |
| NN3           | 33.332 | 47.236 | 244.446 | 2.297    | 6.460    | 99.411  |
| NN4           | 36.596 | 51.839 | 244.549 | 2.330    | 7.227    | 99.977  |
| NN5 Long-only | 37.066 | 52.610 | 244.058 | 2.317    | 6.625    | 99.976  |
| OLS           | 18.286 | 21.621 | 292.987 | 0.252    | 3.491    | 177.870 |
| OLS-3         | 10.601 | 26.089 | 140.764 | 1.300    | 8.418    | 165.672 |
| PLS           | 15.564 | 18.618 | 289.582 | 0.924    | 2.697    | 139.037 |
| LASSO         | 10.688 | 21.596 | 171.447 | 0.198    | 6.335    | 183.899 |
| ENet          | 10.704 | 22.906 | 161.874 | 0.546    | 6.315    | 175.400 |
| Ridge         | 18.072 | 22.237 | 281.523 | −0.191   | 5.606    | 160.958 |
| RF            | 30.519 | 32.603 | 324.275 | 1.609    | 3.677    | 132.221 |
| GBRT          | 29.110 | 31.854 | 316.570 | 2.420    | 7.406    | 113.122 |
| KNN           | 22.645 | 26.285 | 298.448 | 0.357    | 4.041    | 170.251 |
| NN1           | 32.244 | 36.889 | 302.791 | 1.845    | 3.939    | 114.118 |
| NN2           | 27.257 | 32.006 | 295.016 | 1.238    | 4.065    | 156.390 |
| NN3           | 29.515 | 30.304 | 337.401 | 0.829    | 2.254    | 161.401 |
| NN4           | 35.671 | 39.209 | 315.148 | 1.665    | 3.071    | 119.978 |
| NN5           | 34.969 | 39.237 | 308.731 | 1.541    | 2.645    | 123.362 |

Notes: This table reports the out-of-sample performance measures for both long-short and long-only portfolios constructed by all machine learning models. The portfolios are value-weighted and based on the full sample. All measures are derived from the out-of-sample returns from January 2014 to March 2023. "Mean" represents the average predicted weekly return (%). "Std" denotes the standard deviation of the predicted weekly returns (%). "Sharpe" represents the annualized Sharpe ratio (%). "MaxDD" denotes the maximum drawdown (%) of the portfolio.
...
of linear combination weights used to create the  $j$ th predictive component. Thus,  $Z\Omega_K$  is a reduced-dimensional form of the original set of predictors, and the prediction coefficients  $\theta_K$  become a  $K \times 1$  dimensional vector.
...
The algorithm of the RF model is as follows: a tree is generated by a classification and regression tree algorithm. Generate bootstrap samples  $\{(z_{i,t}, r_{i,t+1}), (i, t) \in \text{Bootstrap}(b)\}$  from the original data set. At each step of segmentation, only random subsamples ( $\sqrt{P}$ ) of all features are used. For the  $b$ th tree ( $b \in [1, B]$ ), it can be written as
...
We have considered 5 commonly used activation functions (Sigmoid function, Tanh function, ReLU function, Leaky ReLU function, and Exponential LU function) and find that the Sigmoid function leads to the best result. Therefore, we use this activation function in each layer, and it is given by
...
$$\bar{d}_{12,t+1} = \frac{1}{n_{3,t+1}} \sum_{i=1}^{n_{3,t+1}} \left( \left( \hat{\theta}_{i,t+1}^{(1)} \right)^2 - \left( \hat{\theta}_{i,t+1}^{(2)} \right)^2 \right),$$

where  $\hat{\theta}_{i,t+1}^{(1)}$  and  $\hat{\theta}_{i,t+1}^{(2)}$  represent the prediction error for cryptocurrency  $i$  at time  $t$  utilizing each method, and  $n_{3,t+1}$  is the number of cryptocurrencies in the testing sample (week  $t + 1$ ). Then  $\bar{d}_{12}$  and  $\hat{\sigma}_{\bar{d}_{12}}$  denote the mean and standard error of  $d_{12,t}$  over the testing sample. If the absolute value of the test statistic is greater than the significance level of 5%, we reject the null hypothesis that there is no difference in forecast accuracy between the two methods.

#### A.11 Shapley additive explanations (SHAP)

The SHAP method is a powerful tool in machine learning for explaining model predictions by attributing them to individual input features.<sup>4</sup> It draws inspiration from cooperative game theory, particularly Shapley values introduced by Lloyd Shapley (1953). SHAP values offer a principled and fair way to distribute the contribution of each feature to the model’s prediction, providing insights into the importance and impact of different input variables.
...
<sup>4</sup> For more information, please see <https://shap.readthedocs.io/en/latest/>.
...
Large holders netflow (LHN) measures the net change in cryptocurrency funds held by large investors or entities. This indicator indicates whether influential holders are accumulating or allocating their cryptocurrency assets, reflecting their market sentiment and potential market impact. LHN and its change rate are given by
...
$$RM_t = \sum_{i=1}^{940} \frac{MC_{i,t}}{MC_{MKT,t}} \times r_{i,t},$$

where  $RM_t$  represents the return of the cryptocurrency market on week  $t$ ,  $MC_{i,t}$  represents the market capitalization of the  $i$ th cryptocurrency on week  $t$ ,  $MC_{MKT,t}$  represents the total market capitalization on week  $t$ , and  $MC_{MKT,t} = \sum_{i=1}^{940} MC_{i,t}$ ,  $r_{i,t}$  represents the return of the  $i$ th cryptocurrency on week  $t$ . Then, the market factor is measured by excess market return and is given by

$$RMRF_t = RM_t - RF_t,$$

where  $RF_t$  denotes the risk-free rate on week  $t$ .

The size and momentum factors are derived from market capitalization and the performance of cryptocurrency returns over the previous four weeks. To construct the size factor, we categorize the top 50% of cryptocurrencies by market capitalization as a large portfolio (Big), while the bottom

50% form a small portfolio (Small). For the momentum factor, we use breakpoints at the 30th and 70th percentiles based on the previous four weeks of cryptocurrency returns. Three distinct investment portfolios emerge from this categorization: Up (top 30%), Medium (middle 40%), and Down (bottom 30%). Consequently, six value-weighted portfolios are formed at the intersection of two size and three momentum portfolios. We use SMB (small minus big) to measure the size factor, which is the difference between the average returns of three small cryptocurrency portfolios and three large cryptocurrency portfolios, that is

$$SMB = \frac{1}{3}(Small\ Up + Small\ Medium + Small\ Down) - \frac{1}{3}(Big\ Up + Big\ Medium + Big\ Down).$$
...
$$UMD = \frac{1}{2}(Big\ Up + Small\ Up) - \frac{1}{2}(Big\ Down + Small\ Down).$$

## C Macroeconomic factors

### C.1 Uncertainty indices

The economic policy uncertainty (EPU) index proposed by Baker et al. (2016) quantifies the level of uncertainty of economic policy in the U.S.<sup>5</sup> The EPU return is given by

$$\Delta EPU_{t+1} = \frac{EPU_{t+1} - EPU_t}{EPU_t}.$$

The Twitter-based economic uncertainty (TEU) index derived by Baker et al. (2021) utilizes real-time Twitter data to measure economic uncertainty.<sup>6</sup> Traditional economic indicators and surveys are lagging, and it is difficult to measure the current economic environment accurately. By leveraging Twitter data, the TEU index provides an updated and dynamic measure of economic uncertainty. The TEU return is given by
...
<sup>5</sup> This index is downloaded from the website [https://www.policyuncertainty.com/us\\_monthly.html](https://www.policyuncertainty.com/us_monthly.html).

<sup>6</sup> This index is downloaded from the website [https://www.policyuncertainty.com/twitter\\_uncert.html](https://www.policyuncertainty.com/twitter_uncert.html).
...
Inspired by Aslanidis et al. (2022), we apply 5 keywords (Bitcoin, BTC, cryptocurrency, blockchain, and crypto) to Google Trends and thus construct the Google-Trends-based cryptocurrency index (GTCX).<sup>7</sup>
...
Among the above keywords, “Bitcoin” is the most important keyword. This keyword is the most widely used search term by investors interested in the cryptocurrency market. On the other hand, Bitcoin’s market capitalization is the largest in the cryptocurrency market, and its influence on the market is also the largest. We have examined search terms that are highly relevant to this keyword. Table C.4.1 shows the 10 most relevant search terms and compares their search volume to “Bitcoin.” The top 5 are the most relevant but have lower search volumes. Although the search volume for “mining bitcoin” is high, its relevance is low. The bottom 4 have very low relevance and search volume. The remaining 4 keywords have similar results. Thus, we do not add additional information

<sup>7</sup> The Google Trends series is downloaded from the website <https://trends.google.com/trends/?geo=US>.

to the above keywords.
...
where  $GT_{i,t}$  represents the Google Trends time series of keyword  $i$  on day  $t$ , and  $TGT_t$  represents the sum of time series of all keywords on day  $t$ . The index return on day  $t + 1$  is given by
...
Comparison of weekly out-of-sample prediction for mega-cap cryptocurrencies using Diebold-Mariano test.

| OLS-3 PLS LASSO Ridge ENet OLS −0.675 0.000 −3.192 * 0.001 −3.192 * OLS-3 0.675 −2.516 * 0.676 −2.516 * PLS −3.192 * 0.001 −3.192 * LASSO 3.192 * 0.000 Ridge −3.192 * ENet RF GBRT KNN NN1 | RF GBRT KNN NN1 NN2 NN3 4.365 * 4.081 * 0.680 2.562 * 2.388 * 2.482 * 5.040 * 4.756 * 1.356 3.237 * 3.063 * 3.157 * 4.365 * 4.081 * 0.680 2.562 * 2.388 * 2.482 * 7.556 * 7.273 * 3.872 * 5.754 * 5.579 * 5.674 * 4.364 * 4.081 * 0.680 2.562 * 2.387 * 2.482 * 7.556 * 7.273 * 3.872 * 5.754 * 5.579 * 5.674 * −0.284 −3.684 * −1.802 * −1.977 * −1.882 * −3.401 * −1.519 −1.693 −1.599 1.882 * 1.708 * 1.802 * −0.174 −0.080 | NN4 3.194 * 3.869 * 3.194 * 6.386 * 3.194 * 6.386 * −1.171 −0.887 2.514 * 0.632 | NN5 2.394 * 3.069 * 2.394 * 5.585 * 2.393 * 5.585 * 7.502 * −1.688 1.713 * −0.169 |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
...
Comparison of weekly out-of-sample prediction for large-cap cryptocurrencies using Diebold-Mariano test.

| OLS-3 PLS OLS −1.309 0.000 OLS-3 1.309 PLS | LASSO Ridge ENet −1.419 0.002 −1.419 −0.110 1.311 −0.110 −1.419 0.002 −1.419 | RF 3.383 * 4.692 * 3.383 * | GBRT 2.349 * 3.658 * 2.349 * | KNN 1.645 * 2.955 * 1.645 * | NN1 1.843 * 3.152 * 1.843 * | NN2 1.801 * 3.110 * 1.801 * | NN3 0.985 2.295 * 0.985 | NN4 0.950 2.259 * 0.950 | NN5 1.961 * 3.270 * 1.961 * |
|--------------------------------------------|------------------------------------------------------------------------------|----------------------------|------------------------------|-----------------------------|-----------------------------|-----------------------------|-------------------------|-------------------------|-----------------------------|
| LASSO                                      | 1.421 0.000                                                                  | 4.802 *                    | 3.768 *                      | 3.064 *                     | 3.262 *                     | 3.220 *                     | 2.405 *                 | 2.369 *                 | 3.380 *                     |
| Ridge                                      | −1.421                                                                       | 3.381 *                    | 2.347 *                      | 1.643 *                     | 1.841 *                     | 1.799 *                     | 0.984                   | 0.948                   | 1.959 *                     |
| ENet                                       |                                                                              | 4.802 *                    | 3.768 *                      | 3.064 *                     | 3.262 *                     | 3.220 *                     | 2.405 *                 | 2.369 *                 | 3.380 *                     |
| RF                                         |                                                                              |                            | −1.034                       | −1.738 *                    | −1.540                      | −1.582                      | −2.398 *                | −2.433 *                | 4.802 *                     |
| GBRT                                       |                                                                              |                            |                              | −0.703                      | −0.506                      | −0.548                      | −1.363                  | −1.399                  | −0.388                      |
...
Comparison of weekly out-of-sample prediction for mid-cap cryptocurrencies using Diebold-Mariano test.

|       | OLS-3 PLS    | LASSO  | Ridge | ENet   | RF      | GBRT    | KNN     | NN1     | NN2     | NN3     | NN4     | NN5     |
|-------|--------------|--------|-------|--------|---------|---------|---------|---------|---------|---------|---------|---------|
| OLS   | −0.723 0.000 | −0.701 | 0.000 | −0.701 | 2.509 * | 1.692   | 1.710 * | 1.613   | 1.991 * | 1.678   | 1.417   | 1.908 * |
| OLS-3 | 0.723        | 0.022  | 0.723 | 0.022  | 3.232 * | 2.416 * | 2.434 * | 2.336 * | 2.714 * | 2.402 * | 2.140 * | 2.632 * |
| PLS   |              | −0.701 | 0.000 | −0.701 | 2.509 * | 1.692   | 1.710 * | 1.613   | 1.991 * | 1.678   | 1.417   | 1.908 * |
| LASSO |              |        | 0.701 | 0.000  | 3.209 * | 2.393 * | 2.411 * | 2.314 * | 2.692 * | 2.379 * | 2.118 * | 2.609 * |
| Ridge |              |        |       | −0.701 | 2.509 * | 1.692   | 1.710 * | 1.613   | 1.991 * | 1.678   | 1.417   | 1.908 * |
| ENet  |              |        |       |        | 3.209 * | 2.393 * | 2.411 * | 2.314 * | 2.692 * | 2.379 * | 2.118 * | 2.609 * |
| RF    |              |        |       |        |         | −0.186  | −0.798  | −0.896  | −0.518  | −0.830  | −1.092  | 3.206 * |
| GBRT  |              |        |       |        |         |         | 0.018   | −0.080  | 0.299   | −0.014  | −0.276  | 0.216   |
...
Table E.1 reports the out-of-sample performance for equal-weighted long-short and long-only portfolios, while Figure E.1 shows the dynamics of cumulative returns for both long-short and longonly equal-weighted portfolios. The dynamics of cumulative returns for neural network equalweighted portfolios are provided in Figure E.2.

**Table E.1**

The out-of-sample performance for equal-weighted portfolios on the full sample.
...
Notes: This table reports the out-of-sample performance measures for both long-short and long-only portfolios constructed by all machine learning models. The portfolios are equal-weighted and based on the full sample. All measures are derived from the out-of-sample returns from January 2014 to March 2023. "Mean" represents the average predicted weekly return (%). "Std" denotes the standard deviation of the predicted weekly returns (%). "Sharpe" represents the annualized Sharpe ratio (%).
