...
def get_next_sqrt_price_from_output(sqrt_px96, liquidity, amount_out, zero_for_one):
    ...
    if zero_for_one:  # token1 exits the pool
        return get_next_sqrt_price_from_amount1_down(
            sqrt_px96, liquidity, amount_out, False
        )
    return get_next_sqrt_price_from_amount0_up(sqrt_px96, liquidity, amount_out, False)
...
def compute_swap_step(
    ...
    if exact_in:
        ...
        if zero_for_one:
            amount_out = get_amount1_delta(
                sqrt_price_target_x96, sqrt_price_x96, liquidity, False
            )
        else:
            amount_out = get_amount0_delta(
                sqrt_price_x96, sqrt_price_target_x96, liquidity, False
            )
        ...
        if -amount_remaining >= amount_out:
            ...
            sqrt_price_next_x96 = get_next_sqrt_price_from_output(
                sqrt_price_x96, liquidity, wrap256(-amount_remaining), zero_for_one
            )
    ...
    if not exact_in and amount_out > wrap256(-amount_remaining):
        amount_out = wrap256(-amount_remaining)

    if exact_in and sqrt_price_next_x96 != sqrt_price_target_x96:
        fee_amount = wrap256(amount_remaining) - amount_in
    else:
