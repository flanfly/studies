...
# ///
"""Final analysis: seed selection + tables for both filter-threshold variants."""
import math
import numpy as np
import polars as pl
...
VARIANTS = {"tau009": "seed-{s}.parquet", "tau015": "seed-t15-{s}.parquet"}
...
for variant, pattern in VARIANTS.items() if (VARIANTS := VARIANTS) else []:
    pass
for variant, pattern in VARIANTS.items():
    print(f"\n################ VARIANT {variant} ################")
    print("=== Seed selection on validation folds (DRL_rank EW, first 6 test months) ===")
    val_rows = []
    for s in SEEDS:
        df = pl.read_parquet(pattern.format(s=s)).filter(
    ...
    print(f"BEST SEED on validation: {best_seed}")

    print("\n=== Seed stability, DRL_rank full period (gross) ===")
    stab = []
    for s in SEEDS:
        df = pl.read_parquet(pattern.format(s=s)).filter(pl.col("strategy") == "DRL_rank")
        m = metrics(df["ret_gross"].to_numpy())
        m["seed"] = s
    ...
    df = pl.read_parquet(pattern.format(s=best_seed)).with_columns([
    ...
    print("\n=== Yearly breakdown, final strategy (DRL+filt:MaxDiv), net 5bp ===")
    d = df.filter(pl.col("strategy") == "DRL+filt:MaxDiv").sort("time")
    ts = d["time"].to_numpy().astype("datetime64[Y]")
    r = d["ret_5bp"].to_numpy()
    rows = []
    for y in np.unique(ts):
        m = metrics(r[ts == y])
        m["Year"] = str(y)
